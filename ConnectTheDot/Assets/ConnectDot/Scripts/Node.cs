...
namespace Connect.Core
{
    public class Node : MonoBehaviour
    {
        ...
        public Dictionary<Vector2Int, Node> edges;
        ...
        private Node _lastHighlightedNode;
        public void SolveHighlight()
        {
            // If no connected nodes, disable highlight and return
            ...
            // Lists to track connected nodes
            List<Node> checkingNodes = new List<Node>() { this };
            List<Node> resultNodes = new List<Node>() { this };

            // Traverse connected nodes to build the full list of connected nodes
            while (checkingNodes.Count > 0)
            {
                Node currentNode = checkingNodes[0];
                checkingNodes.RemoveAt(0);

                foreach (var connectedNode in currentNode.ConnectedNodes)
                {
                    if (!resultNodes.Contains(connectedNode))
                    {
                        resultNodes.Add(connectedNode);
                        checkingNodes.Add(connectedNode);
                    }
                }
            }
            ...
            // If exactly two end nodes are connected, highlight the path
            if (endNodes.Count == 2)
            {
                // Enable the highlight and set the correct color
                ...
                _lastHighlightedNode = endNodes[1];
            }
            else
            {
                // If the connection is not valid, disable the highlight
                _highLight.SetActive(false);
                _lastHighlightedNode = null;
            }
